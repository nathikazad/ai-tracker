...
static struct hm01b0_inst_t {
    ...
    pio_sm_config pio_sm_config_sett;
...
int hm01b0_init(const struct hm01b0_config* config)
{
    ...
    if (config->data_bits == 8) {
        ...
        Serial.print("Invalid data bits!\n");
        return -1;
    }
    ...
    uint16_t pio_program_instructions[] = {
        /* 0 */ (uint16_t)pio_encode_pull(false, true),
        /* 1 */ (uint16_t)pio_encode_wait_gpio(false, config->vsync_pin),
        /* 2 */ (uint16_t)pio_encode_wait_gpio(true, config->vsync_pin),
        /* 3 */ (uint16_t)pio_encode_set(pio_y, num_border_px - 1),
        /* 4 */ (uint16_t)pio_encode_wait_gpio(true, config->hsync_pin), // border pixel y
        /* 5 */ (uint16_t)pio_encode_wait_gpio(false, config->hsync_pin),
        /* 6 */ (uint16_t)pio_encode_jmp_y_dec(4),
        /* .wrap_target */
        /* 7 */ (uint16_t)pio_encode_mov(pio_x, pio_osr),
        /* 8 */ (uint16_t)pio_encode_wait_gpio(true, config->hsync_pin),
        /* 9 */ (uint16_t)pio_encode_set(pio_y, num_border_px * hm01b0_inst.num_pclk_per_px - 1),
        /* 10 */ (uint16_t)pio_encode_wait_gpio(true, config->pclk_pin), // border pixel x
        /* 11 */ (uint16_t)pio_encode_wait_gpio(false, config->pclk_pin),
        /* 12 */ (uint16_t)pio_encode_jmp_y_dec(10),
        /* 13 */ (uint16_t)pio_encode_wait_gpio(true, config->pclk_pin),
        /* 14 */ (uint16_t)pio_encode_in(pio_pins, config->data_bits),
        /* 15 */ (uint16_t)pio_encode_wait_gpio(false, config->pclk_pin),
        /* 16 */ (uint16_t)pio_encode_jmp_x_dec(13),
        /* 17 */ (uint16_t)pio_encode_wait_gpio(false, config->hsync_pin),
        /* .wrap */
    };
    ...
    hm01b0_inst.pio_sm_config_sett = pio_get_default_sm_config();

    sm_config_set_in_pins(&hm01b0_inst.pio_sm_config_sett, config->data_pin_base);
    sm_config_set_in_shift(&hm01b0_inst.pio_sm_config_sett, true, true, 8);
    sm_config_set_wrap(
        &hm01b0_inst.pio_sm_config_sett,
...
void hm01b0_read_frame(uint8_t* buffer, size_t length)
{
    ...
    pio_sm_init(config->pio, config->pio_sm, hm01b0_inst.pio_program_offset, &hm01b0_inst.pio_sm_config_sett);
