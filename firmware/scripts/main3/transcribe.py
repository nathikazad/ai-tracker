...
from util import log, clear_previous_time
...
class AudioTranscriber:
    def __init__(self, ble_manager):
        ...
        # ADPCM tables (same as before)
        self.step_table = [7, 8, 9, 10, 11, 12, 13, 14, 16, 17, 19, 21, 23, 25, 28, 31, 34, 37, 41, 45,
    ...
    async def start(self):
        ...
        await self.ble_manager.start_notify(
            self.ble_manager.audio_uuid,
            self._notification_handler
        )

    async def stop(self):
        """Stop listening for audio data."""
        await self.ble_manager.stop_notify(self.ble_manager.audio_uuid)

    def _notification_handler(self, _sender, data):
        """Handle incoming audio data."""
        if data.startswith(b'START'):
            clear_previous_time()
            log("Started receiving audio")
    ...
    def _generate_wav_header(self, sample_rate, bits_per_sample, channels, data_size):
        ...
        return header
    # Rest of the AudioTranscriber methods (_process_audio, _transcribe_wav, _adpcm_decode_block, 
    # _generate_wav_header) remain the same as in your original code
